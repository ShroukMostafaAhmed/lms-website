...
function CalendarComponent() {
    ...
    const { createEvent, isLoading: isCreatingEvent, error: createError } = useCreateEvent();
        ...
        if (calendarData && Array.isArray(calendarData)) {
            ...
            const filtered = calendarData.filter(event => {
                ...
                const eventMonth = event.month || event.Month || 
                                 (event.date ? new Date(event.date).getMonth() + 1 : null);
                const eventYear = event.year || event.Year || 
                                (event.date ? new Date(event.date).getFullYear() : null);
            ...
            const normalizedEvents = filtered.map(event => ({
                day: event.day || event.Day || (event.date ? new Date(event.date).getDate() : null),
                color: normalizeColor(event.color || event.Color || event.bgColor),
                notes: event.notes || event.note || event.title || event.Title,
                id: event.id || event.Id
            })).filter(event => event.day); // Remove events without valid day
...
const handleAddEvent = () => {
    Swal.fire({
        ...
        preConfirm: async () => {
            ...
            const bgClass = colorElement?.getAttribute('data-bg-class');
                        ...
                        >
                            ...
                            {hasEvent && (
                                <div className="absolute bottom-1 left-1/2 transform -translate-x-1/2 flex gap-1">
                                    {events.filter(e => e.day === currentDay).map((dayEvent, eventIndex) => (
                                        <div key={eventIndex} className={`w-2 h-2 ${dayEvent.color} rounded-full shadow-sm`}></div>
                                    ))}
                                </div>
                            )}
                        </div>
    ...
    if (isLoading) {
        return (
            <div className="px-2 flex justify-center items-center min-h-64">
                <div className="text-blue-500">جاري التحميل...</div>
            </div>
        );
    }
        ...
        <div className="px-2">
            <div dir="ltr" className="mt-6 flex justify-between gap-2 items-center">
                <div className="flex items-center space-x-reverse gap-2 lg:gap-4">
                    <button onClick={handleNextMonth} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200">
                        <svg className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
                        </svg>
                    </button>
                    <button onClick={handlePrevMonth} className="w-8 h-8 flex items-center justify-center rounded-full bg-blue-100 text-blue-500 hover:bg-blue-200">
                        <svg className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div className="flex items-center relative">
                    <div className="text-sm cursor-pointer p-2 md:p-3 border border-blue-300 rounded-xl text-blue-500 flex items-center gap-2 md:gap-5 hover:bg-blue-50"
                ...
                <div className="mt-6 space-y-4">
                    {events.map((event, index) => (
                        <div key={`event-${index}`} className="bg-blue-50 rounded-lg p-3 flex justify-between items-center">
                            ...
                            <div className={`${event.color} text-white w-8 h-8 rounded-lg flex items-center justify-center font-bold shadow-sm`}>
                                {event.day}
                            </div>
                        </div>
                    ))}
