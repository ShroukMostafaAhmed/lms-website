...
function CalendarComponent() {
  ...
  useEffect(() => {
    if (calendarData && Array.isArray(calendarData)) {
      ...
      const normalizedEvents = filtered
        .map((event) => ({
          day:
            event.day ||
            event.Day ||
            (event.date ? new Date(event.date).getDate() : null),
  ...
  // Only the handleAddEvent function needs to be updated in your existing CalendarComponent
  ...
  const handleAddEvent = () => {
    Swal.fire({
      ...
      preConfirm: async () => {
        ...
        // Create ISO date string - this matches your API requirement
        ...
        const eventData = {
          date: isoDate, // API expects "date" as ISO string
          notes, // API expects "notes"
          color, // API expects "color"
        };

        try {
          const result = await createEvent(eventData);

          // Update local state if event is in current month/year
          if (
      ...
      <div dir="ltr" className="mt-6 flex justify-between gap-2 items-center">
        ...
        <div className="flex items-center relative">
          ...
          {showDatePicker && (
            <div
              dir="rtl"
              className="absolute top-full mt-1 right-0 bg-white shadow-lg rounded-md p-3 z-10 min-w-48"
            >
              <div className="mb-2">
                <select
